    ...
    const SearchCars = () => {
      ...
      const setCarsmatchingCriteria = async(matching_cars)=>{
        ...
        console.log(Array.from(matching_cars));
        ...
        if(mileageIdx !== 0) {
          ...
          let mincurrmileage = currmileage - 5000;
          if(currmileage === 5000) {
            cars = cars.filter(car => car.mileage < currmileage && car.mileage >= mincurrmileage);
          } else if (currmileage === 10000){
            cars = cars.filter(car => car.mileage < currmileage && car.mileage >= mincurrmileage);
          } else if (currmileage === 15000){
            cars = cars.filter(car => car.mileage < currmileage && car.mileage >= mincurrmileage);
          } else if (currmileage === 20000){
            cars = cars.filter(car => car.mileage < currmileage && car.mileage >= mincurrmileage);
          } else if (currmileage > 20000){
            cars = cars.filter(car => car.mileage >= 20000);
          }
        }
        if(priceIdx !== 0) {
          let currprice = parseInt(document.getElementById('price').value);
          let mincurrprice = currprice - 20000;
          if(currprice === 20000) {
            cars = cars.filter(car => car.price < currprice && car.price >= mincurrprice);
        ...
        <div>
          ...
          <select style={{ marginLeft: '10px', marginRight: '10px' ,paddingLeft: '10px', borderRadius :'10px'}} name="mileage" id="mileage" onChange={SearchCarsByMileage}>
            <option selected value='all'> -- All -- </option>
              ...
              <option value='20001'>20000 or More</option>
          </select>
          ...
          <select style={{ marginLeft: '10px', marginRight: '10px' ,paddingLeft: '10px', borderRadius :'10px'}} name="price" id="price" onChange={SearchCarsByPrice}>
              ...
              <option value='80001'>80000 or More</option>
          </select>
